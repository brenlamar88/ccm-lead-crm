    ...
    <div style={{ minHeight: '100vh', background: 'var(--bg)' }}>
      ...
      <div style={{ maxWidth: 1100, margin: '0 auto', padding: '24px 16px' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 18, flexWrap: 'wrap', gap: 12 }}>
          <div>
            ...
            <p style={{ fontSize: 13, color: 'var(--muted)' }}>{companies.length} {companies.length === 1 ? 'account' : 'accounts'}</p>
          </div>
        ...
        {loading ? (
          <div style={{ textAlign: 'center', padding: '60px', color: 'var(--muted)' }}>⏳ Loading companies…</div>
        ) : filtered.length === 0 ? (
          <div style={{ textAlign: 'center', padding: '60px 20px', background: 'var(--surface)', borderRadius: 16, border: '1px solid var(--line)' }}>
            ...
            <p style={{ color: 'var(--muted)', fontSize: 13 }}>{q ? 'Try a different search.' : 'Add a company or generate leads to populate accounts.'}</p>
          </div>
        ) : (
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(300px, 1fr))', gap: 12 }}>
            {filtered.map(c => (
              <div key={c.id} onClick={() => setDetailId(c.id)} style={{
                background: 'var(--surface)', border: '1px solid var(--line)', borderRadius: 12, padding: 16, cursor: 'pointer',
                ...
                <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
                  ...
                  {c.assigned_to && <span style={{ fontSize: 11, fontWeight: 600, padding: '2px 8px', borderRadius: 20, background: 'var(--brand-50)', color: TEAL_DARK }}>{userName(users, c.assigned_to)}</span>}
                </div>
              </div>
            ))}
          </div>
      ...
      {detail && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', zIndex: 90, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 16 }} onClick={() => setDetailId(null)}>
          <div onClick={e => e.stopPropagation()} style={{ background: 'var(--surface)', borderRadius: 16, padding: 24, maxWidth: 640, width: '100%', maxHeight: '90vh', overflow: 'auto' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 8 }}>
              <div>
                <h2 style={{ fontSize: 18, fontWeight: 700 }}>{detail.name}</h2>
                <p style={{ fontSize: 12, color: 'var(--muted)', marginTop: 2 }}>{[detail.type, [detail.city, detail.state].filter(Boolean).join(', ')].filter(Boolean).join(' · ')}</p>
              </div>
              <button onClick={() => setDetailId(null)} style={{ background: 'var(--surface-2)', border: 'none', borderRadius: 8, width: 32, height: 32, cursor: 'pointer', fontSize: 16 }}>✕</button>
            </div>

            <div style={{ display: 'flex', gap: 8, marginBottom: 16, flexWrap: 'wrap' }}>
              <button onClick={() => setEditCompany(detail)} style={pillBtn('#eff6ff', '#1e40af')}>✎ Edit</button>
              <button onClick={() => setContactModal({ defaultCompanyId: detail.id })} style={pillBtn('var(--brand-50)', TEAL_DARK)}>＋ Add contact</button>
            ...
            {detail.notes && <p style={{ fontSize: 13, color: 'var(--ink-2)', background: 'var(--bg)', borderRadius: 8, padding: '10px 12px', marginBottom: 18, lineHeight: 1.5 }}>{detail.notes}</p>}

            <h3 style={{ fontSize: 13, fontWeight: 700, color: 'var(--ink-2)', marginBottom: 8 }}>Contacts ({(detail.contacts || []).length})</h3>
            <div style={{ marginBottom: 18 }}>
              {(detail.contacts || []).length === 0 ? (
                <p style={{ fontSize: 12, color: 'var(--faint)' }}>No contacts yet.</p>
              ) : (
                (detail.contacts || []).map(ct => (
                  <div key={ct.id} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '8px 10px', border: '1px solid var(--line)', borderRadius: 8, marginBottom: 6 }}>
                    <div style={{ flex: 1 }}>
                      <p style={{ fontSize: 13, fontWeight: 600 }}>
                        {contactName(ct)} {ct.is_primary && <span style={{ fontSize: 10, color: TEAL_DARK, background: 'var(--brand-50)', padding: '1px 6px', borderRadius: 20, marginLeft: 4 }}>Primary</span>}
                      </p>
                      <p style={{ fontSize: 11, color: 'var(--muted)' }}>{[ct.title, ct.email, ct.phone].filter(Boolean).join(' · ')}</p>
                    </div>
            ...
            <h3 style={{ fontSize: 13, fontWeight: 700, color: 'var(--ink-2)', marginBottom: 8 }}>Pipeline deals ({(detail.leads || []).length})</h3>
            {(detail.leads || []).length === 0 ? (
              <p style={{ fontSize: 12, color: 'var(--faint)' }}>No linked deals.</p>
            ) : (
              <div>
                {(detail.leads || []).map(d => (
                  <Link key={d.id} href="/pipeline" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '8px 10px', border: '1px solid var(--line)', borderRadius: 8, marginBottom: 6, textDecoration: 'none', color: 'inherit' }}>
